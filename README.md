...
response
On click function is created for the zipcode search and the location is stored at user input. User input holds the zipcode.
Yelp doesnt allow API calls from the fornt end only the back end.
To get the Json response a proxy was used to request a response from yelp. And returns the response for us. Having to use two apis makes it a little slower.
In the AJAx call the data term is used because instead of putting the information in the url because it is organized cleaner and keeps the url cleaner at the same time.


Giphy API has ids set specifically so that the giph result is not random.
